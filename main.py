...
from data import get_CIFAR as get_data
from util import AverageMeter, ProgressMeter, accuracy, prune_by_mask, update_mask, count_node
...
parser.add_argument('--milestones', metavar='EPOCH', default=[150, 225], type=int, nargs='+',
                    help='list of epoch indices for multi step scheduler '
                    '(must be increasing) (default: 150, 225)')
parser.add_argument('--prune-rate', dest='prune_rate', default=0.8, type=float,
                    help='pruning rate')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            m.weight.mask = torch.ones(
                m.weight.size(), requires_grad=False).cuda(args.gpu)
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, args.milestones, 0.1)

    cudnn.benchmark = True

    train_loader, val_loader, train_sampler = get_data(args)
    ...
    for epoch in range(args.epochs, args.epochs + 60):  # 微调60轮
        ...
        # train for one epoch
        train_finetune(train_loader, model, criterion,
                       optimizer, epoch, args, scaler)
    ...
    if is_main:
        print('任务结束，最佳精度为', best_acc1.item())
    count_node(model)
